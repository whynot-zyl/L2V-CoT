...
for task in "${tasks[@]}"; do
    for model in "${models[@]}"; do
        ...
        for lambda_inj in 0.1 0.2; do
            # for inj_layer in $(seq 1 26); do
            for inj_layer in $(seq 1 2); do
                ...
                # torchrun --nproc-per-node=${GPU} --master-port=1231 run_txy_representation.py \
                #     --data "$task" \
                #     --model "$model" \
                #     --verbose \
                ...
                python run_txy_representation.py \
                    --data "$task" \
                    --model "$model" \
                    --verbose \
                    --work-dir "${OUTPUT_DIR}/base_models_${inj_layer}_${lambda_str}" \
